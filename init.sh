...
function initRollup {
    echo -n "${CYAN}Vamos a lanzar base Rollup d3 ¿ya has creado una carpeta? (s/n): "
    ...
    then
        echo -e "${GREEN} Perfecto, vamos a comenzar con Base Rollup d3"
        launch;
    else
        echo -n "${PURPLE}Pues elige el nombre de tu proyecto: ${YELLOW}"
        read -r var_name &&
        mkdir "$var_name" &&
        cd "$var_name" &&
        launch
